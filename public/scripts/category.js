...
if (modal) {
    ...
    if (minRating) {
        ...
        for (let i = 0; i < ratingInputs.length; i++) {
            if (parseFloat(ratingInputs[i].value) >= parseFloat(minRating)) {
    ...
    if (minExperience) {
        const experienceInputs = modal.querySelectorAll('input[name="experience"]');
        for (let i = 0; i < experienceInputs.length; i++) {
            if (parseInt(experienceInputs[i].value) >= parseInt(minExperience)) {
                experienceInputs[i].checked = true;
            }
        }
    }
}
